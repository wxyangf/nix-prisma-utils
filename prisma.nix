{
  ...
  fromPnpmLock = path: let
    ...
    splitCharacter =
      if nixpkgs.lib.strings.hasPrefix "lockfileVersion: 5" pnpmLock
      then "/"
      else "@";
    version =
      ...
      0;
    commit = builtins.elemAt (builtins.match "([A-Za-z0-9]+)'?" (nixpkgs.lib.lists.last (nixpkgs.lib.strings.splitString "." version))) 0;
  in
    fromCommit commit;
